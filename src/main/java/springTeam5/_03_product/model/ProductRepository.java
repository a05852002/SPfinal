...
//	條件搜尋
	@Query(value = "select * from Product where IIF(?1=0,?1,prodClass)=?1 and prodPrice >= ?2 and prodPrice <= ?3 and prodName like concat('%',?4,'%') order by prodID", nativeQuery = true)
	public List<Product> findAllByOrderByProdID(Integer type, Integer low, Integer high, String name);

	@Query(value = "select * from Product where IIF(?1=0,?1,prodClass)=?1 and prodPrice >= ?2 and prodPrice <= ?3 and prodName like concat('%',?4,'%') order by ProdPrice DESC", nativeQuery = true)
	...
	@Query(value = "select * from Product where IIF(?1=0,?1,prodClass)=?1 and prodPrice >= ?2 and prodPrice <= ?3 and prodName like concat('%',?4,'%') order by ProdPost DESC", nativeQuery = true)
	public List<Product> findAllByOrderByProdPostDesc(Integer type, Integer low, Integer high, String name);

	@Query(value = "select * from Product where IIF(?1=0,?1,prodClass)=?1 and prodPrice >= ?2 and prodPrice <= ?3 and prodName like concat('%',?4,'%') order by prodUpdate DESC", nativeQuery = true)
